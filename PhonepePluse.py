...
def about_page():
    ...
    col1.image(Image.open("C:/Users/vinud/DATA SCIENCE/GUVI PROJECT 2/image1.jpg"), width=500)
    with col1:
    ...
    with col2:
        st.video("C:/Users/vinud/DATA SCIENCE/GUVI PROJECT 2/pulse-video.mp4")

# HOME PAGE
def home_page():
    col1,col2 = st.columns(2)
    with col1:
        st.video("C:/Users/vinud/DATA SCIENCE/GUVI PROJECT 2/upi.mp4")
    with col2:
        ...
        st.markdown("## :violet[Created By] : Vinod Kumar K")
        st.markdown("[Inspired from](https://www.phonepe.com/pulse/)")
        st.markdown("[Githublink](https://github.com/vinudrago)")
        st.markdown("[LinkedIn](https://www.linkedin.com/in/vinu-drago-506854276/)")
...
def analysis_page():
    ...
    if select == "INDIA":
        ...
        with tab2:
            ...
            cursor.execute(f"SELECT State, SUM(User_Count) FROM agg_user WHERE Year = '{in_us_yr}' AND Quarter = '{in_us_qtr}' GROUP BY State;")
            ...
            cursor.execute(f"SELECT SUM(User_Count), AVG(User_Count) FROM agg_user WHERE Year = '{in_us_yr}' AND Quarter = '{in_us_qtr}';")
    ...
    if select == "STATES":
        ...
        with tab4:
            ...
            # User Analysis Bar chart query
            cursor.execute(f"SELECT Quarter, SUM(User_Count) FROM agg_user WHERE State = '{st_us_st}' AND Year = '{st_us_yr}' GROUP BY Quarter;")
...
def insights_page():
    ...
    #10
        ...
        cursor.execute(
            "SELECT DISTINCT Brands, SUM(User_Count) as Total FROM agg_user GROUP BY Brands ORDER BY Total DESC LIMIT 10");
        data = cursor.fetchall()
